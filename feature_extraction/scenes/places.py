...
def build_model(include_top=True, weights='places',
              input_tensor=None, input_shape=None,
              pooling=None, classes=365):
    ...
    with tf.device("cpu:0"):
        ...
        if weights == 'places':
            ...
            weights_path = 'scenes/vgg16-places365_weights_tf_dim_ordering_tf_kernels.h5'
        ...
        file_name = 'scenes/categories_places365.txt'
        ...
        file_name = 'scenes/IO_places365.txt'
        if not os.access(file_name, os.W_OK):
        ...
        with open(file_name) as class_file:
            for line in class_file:
                ...
                label  = line[0][3:]
                macro  = 'INDOOR' if int(line[1]) == 1 else 'OUTDOOR' 

                # INDOOR (residential=1, commercial=2), OUTDOOR (nature=1, urban=2)
                if macro == 'INDOOR':
                    macro2 = 'RESIDENTIAL' if int(line[2]) == 1 else 'COMMERCIAL' 
                else:
                    macro2 = 'NATURE' if int(line[2]) == 1 else 'URBAN' 

                classes_macro[label] = (macro, macro2)

        return model, classes, classes_macro


def forward(model, classes, image):
    ...
    with tf.device("cpu:0"):
        ...
        top_pred = np.argsort(preds)[::-1][0]#[0:predictions_to_return]

        return top_pred, classes[top_pred]